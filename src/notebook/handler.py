...
def show(book, mode=None):
    ...
    if mode not in ["date", "alp", "size"] and mode:
        ...
        if list_of_notes:
            ...
            print(table)
            print(
                "Щоб працювати з нотаткою треба обрати одну: search <назва нотатки|теги>, help - список команд"
            )
        else:
...
def tags(book, command, note):
    if command not in ["change", "clean", "add"]:
        ...
        if command == "clean":
            ...
            new_tags = input(
                "Щоб додати, введіть теги через пробіл, (натисніть Enter, щоб відмінити)\n>>> "
            )
            ...
            new_tags = input(
                "Щоб змінити, введіть теги через пробіл, (натисніть Enter, щоб відмінити)\n>>> "
            )
            new_tags = new_tags.split(" ")
            new_tags = [t for t in new_tags if t]  # Видаляє пусті теги ("")
            if new_tags:
...
def search(book, value, search_result=True):
    ...
    if not result:
        ...
        if search_result:
            ...
            print(table)
        input_2 = ""
        while not input_2.startswith(("change")) and input_2 not in [
            "delete",
            "close",
            "tags add",
            "tags change",
            ...
            input_2 = (
                completer_input(">>> (editnote) ", COMMAND_INPUT_MENU)
                .casefold()
                .strip()
            )
            ...
            if input_2.startswith(("show", "search", "add", "help")):
                ...
                if not input_2.startswith(("change")) and input_2 not in [
                    "delete",
                    "close",
                    "tags add",
                    "tags change",
        ...
        print("Результат пошуку")
        print(table)
        choose_note_name = input(
            "Щоб працювати з нотаткою, терба обрати одну\nБудь ласка, укажіть повну назву необхідної нотатки (cont - відмінити) \n>>> "
        )

        while (
            choose_note_name not in [note.name for note in result]
            and choose_note_name != "close"
            and choose_note_name != "cont"
        ):
            print("Результат пошуку")
            print(table)
            choose_note_name = input(
        ...
        search(
            book, choose_note_name
        )  # рекурсивно викликаємо функцію коли обрали одну нотатку
